...
if(isset($_POST['likedpost'])){
	$lkpost=new LikedPosts();
	$lkpost->showLikedPosts();
}
if(isset($_POST['unlikeallposts'])){
	global $con;
	$user_id=$_POST['user_id'];
	$run_likedata=mysqli_query($con,"select * from likes where liked_by='$user_id'");
	while($row_data=mysqli_fetch_array($run_likedata)){
		$post_id=$row_data['post_id'];
		$notify_id=$row_data['likenotify_id'];
		$run_postdata=mysqli_query($con,"select * from user_posts where file_id='$post_id'");
		$row=mysqli_fetch_array($run_postdata);
		$n=$row['likes'];
		mysqli_query($con,"update user_posts set likes=$n-1 where file_id='$post_id'");
		mysqli_query($con,"delete from notifications where notify_id='$notify_id'");
		mysqli_query($con,"delete from likes where liked_by='$user_id' AND likenotify_id='$notify_id'");
	}
}
...
class LikedPosts{
	function showLikedPosts(){
		...
		$select_post="select * from likes where liked_by='$sessionuserid' ORDER BY liked_date DESC";
				...
				<div class='panel panel-default' id='profsavedtab'>
					<div class='panel-body'>
						<div class='alert alert-danger'>
							<p style='text-align:center;font-family:bold;font-size:1.5em;'>No Posts Liked Yet!</p>
						</div>
					</div>	
				</div>
			";
